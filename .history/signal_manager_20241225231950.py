...
class SignalManager(QObject):
    ...
    def connect_signal_async(self, signal_name, receiver, method_name, *args):
        signal = getattr(self, signal_name, None)
        for arg in args:
            print(f"....argumenti su {arg}")
        if signal:
            method = getattr(receiver, method_name, None)
            if callable(method):
                # Pravljenje asinhronog zadatka sa prosleđenim argumentima
                signal.connect(lambda: asyncio.create_task(method(*args)))
            

    def emit_signal(self, signal_name, *args):
        # Generičko emitovanje signala prema imenu signala
        print(f".......signal je {signal_name}")
        signal = getattr(self, signal_name, None)
        for arg in args:
            print(arg)
        print(f"emituje sa signala ....{signal}")
        if signal:
            signal.emit(*args)
        else:
