...
class FileExplorerView(QWidget):
    ...
    def populate_files(self):
        ...
        for file_name, file_info in self.files_data.items():
            ...
            self.tree_view.resizeColumnToContents(0)  # Prilagodi širinu prve kolone prema sadržaju
            self.tree_view.setColumnWidth(0, max(self.tree_view.columnWidth(0), 155))  # Osiguraj minimalnu širinu

            # Prilagodi veličinu prozora na osnovu širine kolona
            total_width = sum(self.tree_view.columnWidth(i) for i in range(self.model.columnCount())) + 50
            self.setMinimumWidth(total_width)
            self.setMinimumHeight(400)  # Postavi minimalnu visinu, prilagodi po potrebi
