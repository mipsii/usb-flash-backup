...
class UiLogic(QObject):
    ...
    def format_backup_datetime(self, date_str):
        ...
        try:
            ...
            return date_str  # Ako nešto pođe po zlu, vrati originalni naziv fajla
    
    self.backup_list.itemClicked.connect(self.on_backup_clicked)

    def on_backup_clicked(self, item):
        backup_index = item.data(Qt.UserRole)
        print(f"Kliknut backup: {item.text()} (indeks: {backup_index})")
        #asyncio.create_task(self.main_windows.ui_logic.recursive_backup_merge(self.main_windows.ui_logic.history, backup_index))
