...
class MainWindow(QMainWindow):
    ...
    async def analyz_files(self,usb_backup, sserial_number):
        ...
        async with self.lock:  # Ovo osigurava da samo jedan zadatak bude aktivan u svakom trenutku
            backup_path = self.usb_class.usb_folder.find_usb_folder(sserial_number)
            print(f" USB putanja  je { usb_backup  }")
            print(f"putanja BACKUP  je { backup_path }")
