...
class UiLogic(QObject):
    ...
    def _initiate(self):
        ...
        self.managerList.usb_list_updated.connect(self.update_backup_list)
        pass
    ...
    def on_item_clicked(self, item):
        ...
        serial_number, usb_data = self.usb_class.find_device(serial_number=serial_number)
        self.ui.update_usb_info(usb_data{"Label"}, usb_data["Active"], is_registered=True):
        if usb_data:
            # Prikazuješ podatke u centralnom delu prozora
            self.ui.central_name_label.setText(usb_data["name"])  # Ime USB-a
