...
class USBDeviceManager:
    ...
    def get_connected_usb_devices(self, device_node):
        ...
        # Ovo će zavisiti od sistema i biće implementirano za svaki sistem posebno.
        # Na primer, za Linux možete koristiti `lsblk` ili `lsusb` za listu USB uređaja.
        ...
        print(f" sa cime se puni  { device["Serial number"],device["Label"],sdevice["Device"],device["Mount Point"]}")
        #self.usb_class.usb_info_manager.add_usb_info(device["Serial number"],device["Label"],device["Device"],device["Mount Point"], register_bool = True, current_bool = True, active_bool = True)
        
        #self.usb_class.usb_info_manager.update_current_usb()
        return device
    ...
    def find_device(self, device=None, serial_number=None, mount_point=None, label=None):
        ...
        print(f"pretrazujem recnik :  {usb_info}")
