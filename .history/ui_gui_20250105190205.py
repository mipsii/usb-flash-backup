...
class UiGui:
    ...
    def setupUi(self, MainWindow):
        ...
        self.set_label_message( _("USB nije umetnut"))
        self.set_status_message("Whait", color = "gray")
        self.changeButton(_("Promeni status"), False)
    ...
    def usb_inserted(self, serial_number, usb):
        ...
        if serial_number:
            self.label.setText("")
            self.translation_manager.add_widget(self.label, _(f"USB: { usb['Label'] } Serijski broj: { serial_number }"))
            self.statusLabel.setText("")
            self.translation_manager.add_widget(self.statusLabel, _(f"USB je povezan na {usb['Device']}."))
            self.changeButton.setVisible(True)
        else:
            self.label.setText("")
            self.translation_manager.add_widget(self.label, _(f"USB nije umetnut"))
            self.statusLabel.setText("")
    ...
    def update_usb_status(self, serial_number , usb):
        ...
        if usb["Is current"]:
            self.label.setText("")
            self.translation_manager.add_widget(self.label, _(f"USB: { usb['Label']} <br> (Serijski broj: {serial_number})"))
            ...
            if usb["Is registar"]:
                if usb["Active"]:
                    ...
                    self.translation_manager.add_widget(self.set_button_state, _("Backup"))
                    self.is_register_action = False
                    self.statusLabel.setStyleSheet("color: green;")
                    self.set_status_message("", color='green')
                    self.translation_manager.add_widget(self.set_status_message, _("USB je registrovan. <br> Spreman za backup."))
                else:
                    self.changeButton.setVisible(False)
                    self.is_register_action = True
                    self.statusLabel.setStyleSheet("color: orange;")
                    self.set_status_message("", color='orange')
                    self.translation_manager.add_widget(self.set_status_message, _("USB je registrovan. <br> nije aktivan."))

            else:   
                self.set_button_state("")
                self.translation_manager.add_widget(self.set_button_state, _("Registruj& mount USB"))
                self.statusLabel.setStyleSheet("color: red;")
                self.set_status_message("", color='red')
                self.translation_manager.add_widget(self.set_status_message, _("USB nije registrovan. Registrujte ga."))
        else:
            self.label.setText("")
            self.translation_manager.add_widget(self.label, _("nema USB aktivnih"))
            self.statusLabel.setText("")
            self.changeButton.setVisible(False)