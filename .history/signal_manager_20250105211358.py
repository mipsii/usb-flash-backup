...
class SignalManager(QObject):
    ...
    _instance = None  
    # Definicija signala
    sigAnalyzeFiles = Signal()
    sigHistoryChanged = Signal(dict)
    sigUpDateGui = Signal(str)
    # signal_gui = Signal(str, dict)
    sigGui = Signal(str,dict)
    # send_merged_state = Signal()
    sigSendMergedState = Signal()
    # usb_status_changed = Signal(str, dict)  # Signal sa serijskim brojem i detaljima o USB-u
    sigUsbStatusChanged = Signal(str, dict)
    # usb_action =Signal()
    sigUsbAction = Signal()
    ...
    # finished = Signal()
    sigFinished = Signal()
    # signal_change_language = Signal(str)
    sigChangeLanguage = Signal(str)
    sigRemoveUsb = Signal(str)
    ...
    def connect_signal(self, signal_name, receiver, method_name):
        ...
        if signal:
            ...
            if callable(method):
                ...
                print(f"[CONNECT-SYNC] Signal: ")
                print(f"{signal_name} -----> {method.__qualname__}")
    ...
    def emit_signal(self, signal_name, *args, callback=None):
        # Generičko emitovanje signala prema imenu signala
        print(f"[EMIT] Signal: {signal_name} | ")
        print(f"[DATA]: {args}")
        signal = getattr(self, signal_name, None)
        if signal:
            signal.emit(*args)
