...
class UiGui:
    ...
    def setupUi(self, MainWindow):
        ...
        self.left_widget.setWindowTitle("Buckups")
        # # Naslov iznad liste, sa stilom "tab"
        
        # Okvir za naslov (iznad liste)
        title_frame = QFrame(self.left_widget)
        title_frame.setStyleSheet("""
            background-color: #2c3e50;  /* Tamno plava pozadina */
            border-radius: 3px;
            
        """)

        # Naslov u okviru
        title_label = QLabel("Backups", title_frame)
        title_label.setStyleSheet("""
            font-size: 16px;
            color: #f9f9f9;
            text-align: center;
            padding: 0;  /* Bez dodatnog padding-a */
            margin: 0;   /* Bez margina */
        """)
        # Horizontalni layout za postavljanje naslova
        title_layout = QHBoxLayout(title_frame)
        title_layout.addWidget(title_label)
        title_layout.setAlignment(Qt.AlignCenter)

        # Automatski postavite visinu okvira na visinu sadržaja naslova
        title_frame.setFixedHeight(title_label.sizeHint().height())

        # Dodavanje okvira sa naslovom
        self.left_layout.addWidget(title_frame)
        title_frame.move(-10, -10)  # Pozicija naslova (gore, uz left_widget)
