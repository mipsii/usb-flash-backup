...
class FileChangeManager:
    def __init__(self, backup_path, usb_path):
        ...
        self.popup_window = None  # Referenca na popup
        
        self.file_collector = FileCollector(self)
        self.backup_manager = USBBackupManager(self)
        
        print(f" usb putanja  je { self.usb_path }")
        print(f"putanja cakup je { self.backup_path }")
    ...
    def initiate_backup_with_popup(self):
        # Upoređivanje fajlova
        print("Upoređivanje fajlova...")    
        self.compare_files()
        ...
        if popup.exec() == QDialog.DialogCode.Accepted:
            asyncio.run(self.sync(self.usb_path, self.backup_path))
        else:
            print("Backup otkazan.")
        self.clear_changes()
        self.popup_window = None  # Resetuje referencu nakon zatvaranja popup-a
    ...
    def compare_files(self):
        ...
        self.modified_files = [
            ...
            if self.usb_files[file]['modification_time'] > self.backup_files[file]['modification_time']
        ]
    ...
    async def backup_files(self, target_index=None):
        ...
        """
        history = self.file_collector.load_backup_history()
        #return await asyncio.to_thread(self.file_collector.recursive_backup_merge(history, target_index=target_index))
        self.backup_file = await self.file_collector.recursive_backup_merge(history, target_index=target_index)
