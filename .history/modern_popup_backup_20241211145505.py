...
class ModernBackupPopup(QDialog):
    ...
    def _load_list(self, list, files):
        for file in files:
                ...
                # Povezivanje hover događaja sa itemom
                item.hovered.connect(lambda item=item: self._on_item_hovered(item))  # Pozivanje funkcije kada je stavka "hovered"
    ...
    def _on_item_hovered(self, item):
        ...
        # Ako je putanja predugačka, podeli je na više redova
        max_length = 50  # Ovaj broj možete prilagoditi na osnovu željene dužine pre preloma
        if len(file_path) > max_length:
            # Podeli putanju u delove i ubaci <br> između delova
            file_path = "<br>".join([file_path[i:i+max_length] for i in range(0, len(file_path), max_length)])
        
        # Prikazivanje prelomljene putanje u status baru koristeći HTML
