...
class FileExplorerView(QWidget):
    def __init__(self, files_data):
        ...
        self.start_path = QDir.rootPath()  # Čuvaj početni root direktorijum
    ...
    def navigate_back(self):
        if self.history:
            ...
            self.left_tree_view.setRootIndex(self.left_model.index(self.current_path))
        else:
            print("Nema više foldera u istoriji. Vraćam se na root.")
            self.current_path = self.start_path  # Vrati na root
            self.left_tree_view.setRootIndex(self.left_model.index(self.start_path))
