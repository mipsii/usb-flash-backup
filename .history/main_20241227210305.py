...
class MainWindow(QMainWindow):
    ...
    def _signals(self):
        ...
        self.signal_manager.connect_signal("",self, "manipulation_usb")
    ...
    def handle_usb_event(self, action, deviceID):
        ...
        if action == "add":
            ...
            self.usb_class.update_usb_info(serial_number, {'current_bool':True})
            self.usb_class.update_usb_info(serial_number, {'active_bool' :True})
            # Ažuriranje GUI-ja
    ...
    def manipulation_usb(self, serial_number):
        if self.usb_class.usb_info[serial_number]["Is registar"]:
            self.signal_manager.emit_signal("backup_signal")
        else:
            self.signal_manager.emit_signal("registar_signal",serial_number, self.usb_class.usb_info[serial_number])
