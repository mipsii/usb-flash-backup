...
class FileCollector(QObject):
    ...
    @merged_state.setter
    def merged_state(self, value):
        if self._merged_state != value:
            self._merged_state = value
    ...
    def prosledjeni(self, history, current_index, target_index, merged_state):
        if current_index > target_index or current_index >= len(history):
                ...
                #####self.signal_manager.emit_signal("sigSendMergedState", merged_state)
    ...
    async def prenos(self, backup_path, history, target):
         await asyncio.create_task(self.recursive_backup_merge( backup_path, history, target_index=target))
         self.signal_manager.emit_signal("sigFinished")
