...
class ModernBackupPopup(QDialog):
    ...
    def __init__(self, modified_files=None, new_files=None, deleted_files=None, parent=None):
        ...
        # Omogućavanje praćenja miša na listama
        self.modified_files_list.setMouseTracking(True)
        self.new_files_list.setMouseTracking(True)
        self.deleted_files_list.setMouseTracking(True)

        # Povezivanje događaja miša
        self.modified_files_list.mouseMoveEvent = self._on_mouse_move
        self.new_files_list.mouseMoveEvent = self._on_mouse_move
        self.deleted_files_list.mouseMoveEvent = self._on_mouse_move
    ...
    def _load_list(self, list, files):
        for file in files:
                ...
                # Pozivanje funkcije kada je stavka "hovered"
                
                list.addItem(item)
        return list
    
    def _on_mouse_move(self, event):
        # Detekcija stavke na osnovu pozicije miša
        list_widget = self.sender()  # Prepoznaje koja lista je izazvala događaj
        item = list_widget.itemAt(event.pos())
        if item:
            file_path = item.data(1)  # Dohvatanje pune putanje iz stavke
            # Ako je putanja predugačka, formatiraj je za status bar
            max_length = 50
            if len(file_path) > max_length:
                file_path = "<br>".join([file_path[i:i+max_length] for i in range(0, len(file_path), max_length)])
            self.status_bar.showMessage(f"<html>{file_path}</html>")
        else:
            self.status_bar.clearMessage()  # Očisti status bar ako nema stavke
