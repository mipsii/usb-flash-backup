...
class UiLogic(QObject):
    ...
    def __init__(self, main_window):
        ...
        self.signal_manager.connect_signal("sigUsbStatusChanged", self, "usb_status_to_add_usb_wrapper")
        self.signal_manager.connect_signal("sigHistoryChanged", self, "update_backup_list")
        self.signal_manager.connect_signal("sigUsbAction", self, "")   
    ...
    def on_item_clicked(self, item):
        ...
        if self.selected_item == serial_number:
            ...
            #self.ui.usb_remove()  # Ažurira GUI za deaktivaciju
            usb_data[serial]["Is current"] = False
            self.signal_manager.emit_signal("sigRemoveUsb", serial, callback= lambda *args:
                self.signal_manager.emit_signal("sigUsbStatusChanged",serial, usb_data))
            return
        ...
        self.signal_manager.emit_signal("sigUpDateGui", serial)
        print(f"Signal emitovan sa: {serial}")
        print(f" data jew .... { usb_data }")
        # Poziva direktno ažuriranje GUI komponente
        self.signal_manager.emit_signal("sigGui", serial_number, usb_data)
    ...
    def on_backup_clicked(self, item):
        ...
        self.main_window.backup_files_index(backup_path ,backup_index)
