...
class FileChangeManager(QObject):
    ...
    def _signals(self):
        ...
        self.signal_manager.connect_signal_async("sigAnalyzeFiles", self,  "analyze_changes" )
        self.signal_manager.connect_signal_async("backup_signal", self, "initiate_backup_with_popup")

    def spisak(self):
        if self.backup_path:
            self.history = self.load_backup_history()
        else:
            print("-----nema jos backupnfiles")   
    ...
    def history(self, value):
        if self._history != value:
            ...
            print("......signal sa history ")
            QTimer.singleShot(0, lambda: self.signal_manager.emit_signal("sigHistoryChanged",self.history))
    ...
    async def initiate_backup_with_popup(self):
        ...
        self.history = self.load_backup_history()
        print(f".........Nova vrednost histrory je ...{self._history} ")
    ...
    async def backup_files(self, target_index=None):
        ...
        if self.backup_path:
            ...
            print("NE POSTOJI JER JE PRVI PUT")
            self.backup_file = {}
