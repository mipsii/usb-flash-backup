...
class UsbInfoManager:
    ...
    def get_usb_info(self):
        """Vraća informacije o trenutnom uređaju za prikaz u GUI-ju."""
        print("uzima vrednaoti usb info")
    ...
    def adjust_list_height(self, usb_list_widget):
        """
        Prilagođava visinu QListWidget liste na osnovu broja uređaja.
        """
        num_items = usb_list_widget.count()
        max_visible_items = 3
        item_height = 30
        if num_items > 0:
            usb_list_widget.setVisible(True)
            usb_list_widget.setFixedHeight(min(num_items, max_visible_items) * item_height)
        else:
            usb_list_widget.setVisible(False)  # Sakrijemo ako nema uređaja
        return usb_list_widget
