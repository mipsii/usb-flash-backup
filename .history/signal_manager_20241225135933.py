import asyncio
from PySide6.QtCore import QMetaObject, Qt, QObject, Signal, QRunnable, QThread
...
class SignalManager(QObject):
    ...
    def start_thread(self, func, *args, tag=None, **kwargs):
        ...
        if tag:
            thread.signal_manager.finished_signal.connect(lambda result: self.emit_signal_by_tag(tag, result))
        else:
            thread.signal_manager.finished_signal.connect(self.thread_finished.emit)
...
class WorkerThread(QThread):
    ...
    def run(self):
        ...
        self.signal_manager.emit_signal("finished_signal",result)
