...
class UiLogic:
    ...
    def on_change_button_clicked(self):
        ...
        if self.ui.changeButton.text() == "Backup":
            ...
            sserial_number, details = self.main_window.usb_class.usb_info_manager.get_current_usb_info()
            label = details["Label"]
            self.main_window.usb_class.usb_folder.create_hidden_file_with_serial_number(label, sserial_number)
            self.main_window.usb_class.usb_info_manager.update_usb_info(sserial_number, {"Is registar": True})
