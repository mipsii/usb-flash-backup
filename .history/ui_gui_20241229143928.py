...
class UiGui:
    ...
    def setupUi(self, MainWindow):
        ...
        self.lang_selector.addItems(["English", "Srpski", "Deutsch"])
        self.lang_selector.currentIndexChanged.connect(self.change_language) 
        self.MainWindow.addWidget(self.lang_selector)
...
def change_language(self, index):
        ...
        load_translation(new_language)

def load_translation(language):
    lang = gettext.translation('app', localedir='locale', languages=[language], fallback=True)
    lang.install()
    global _
    _ = lang.gettext
        ...
        if usb["Is current"]:
            ...
            if usb["Is registar"]:
                if usb["Active"]:
                    ...
                    self.set_status_message("USB je registrovan. <br> Spreman za backup.", color='green')
                else:
                    self.changeButton.setVisible(False)
                    self.is_register_action = True
                    self.statusLabel.setStyleSheet("color: orange;")
                    self.set_status_message("USB je registrovan. <br> nije aktivan.", color='orange')
            else:
                self.set_button_state("Registruj& mount USB")
                self.statusLabel.setStyleSheet("color: red;")
                self.set_status_message("USB nije registrovan. Registrujte ga.", color='red')
        else:
            self.label.setText("nema USB aktivnih")
            self.statusLabel.setText("")
            self.changeButton.setVisible(False)