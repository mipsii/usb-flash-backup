...
class ModernBackupPopup(QDialog):
    ...
    def __init__(self, modified_files=None, new_files=None, deleted_files=None, parent=None):
        ...
        # Omogućavanje praćenja miša i povezivanje događaja
        self._enable_list_behavior(modified_files_list)
        self._enable_list_behavior(new_files_list)
        self._enable_list_behavior(deleted_files_list)
        ...
        # self._enable_hover(modified_files)
        # self._enable_hover(new_files_list)
        # self._enable_hover(deleted_files_list)
        ...
        # Povezivanje hover događaja
        self.new_files.itemEntered.connect(self._on_item_hovered)
        self.modified_files.itemEntered.connect(self._on_item_hovered)
        self.deleted_files.itemEntered.connect(self._on_item_hovered)
    ...
    def _load_list(self, list, files):
        for file in files:
                ...
                item.setData(0, file)  # Skladištenje pune putanje u stavku
                list.addItem(item)
                
        return list
    
    def _enable_list_behavior(self, list_widget):
        """
        Povezuje hover i klik događaje za QListWidget.
        """
        list_widget.setMouseTracking(True)  # Omogućava praćenje miša
        list_widget.itemEntered.connect(self._on_item_hovered)  # Hover događaj
        list_widget.itemClicked.connect(self._on_item_clicked)  # Klik događaj

    def _on_item_hovered(self, item):
        """
        Prikazuje punu putanju u status baru kada se pređe mišem preko stavke.
        """
        file_path = item.data(0)  # Dobijanje pune putanje iz stavke
        self.status_bar.showMessage(f"Putanja (hover): {file_path}")

    def _on_item_clicked(self, item):
        """
        Prikazuje punu putanju u status baru kada se klikne na stavku.
        """
        file_path = item.data(0)  # Dobijanje pune putanje iz stavke
        self.status_bar.showMessage(f"Putanja (klik): {file_path}")
